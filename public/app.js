document.addEventListener("DOMContentLoaded", () => {
  ...
  if (highlightJsAvailable) {
    ...
    console.warn(
      "highlight.js is not available. Code highlighting will be disabled.",
    );
  }
  ...
  if (mermaidAvailable) {
    ...
    console.warn(
      "mermaid is not available. Diagram rendering will be disabled.",
    );
  }
  ...
  let currentFont = localStorage.getItem("font") || "serif";
  ...
  themeOptions.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      btn.classList.add("active");
      tempTheme = btn.dataset.theme;
      applyTheme(tempTheme);
    });
  });

  // Font selection (Live Preview)
  fontFamilySelect.addEventListener("change", () => {
    tempFont = fontFamilySelect.value;
    applyFont(tempFont);
  });
  ...
  function loadDirectory(path, parentElement = null) {
    fetch(
      `/api/directory?path=${encodeURIComponent(path)}&includeHidden=${showHiddenFiles}`,
    )
    ...
    items.forEach((item) => {
      ...
      if (item.type === "directory") {
        ...
        header.addEventListener("click", () => {
          ...
          document
            .querySelectorAll(".directory-item-header.active")
            .forEach((el) => {
  ...
  function processMermaidDiagrams() {
    const codeBlocks = markdownPreview.querySelectorAll(
